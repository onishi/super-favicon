...
- `SuperFaviconBrowser/AppIcon.icon` — アプリアイコン（Icon Composer 形式。コンパスの SVG に背景グラデーション・シャドウを設定）。`project.yml` 変更後と同様、ファイルを追加/変更したら `xcodegen generate` で `.xcodeproj` に反映すること
